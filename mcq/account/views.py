...
class OTPValidation(APIView):
   ...
   def post(self, request, format= None):
      ...
      serializer  = OTPValidationSerializer(data = request.data,context={'user':user})
      if serializer.is_valid():
         return Response({'message':'OTP code validated  successfully !', 'success':'true'}, status=status.HTTP_200_OK)
      else:
...
class UserPasswordResetView(APIView):
  ...
  def post(self, request, format=None):
    ...
    if serializer.is_valid():
      ...
      return Response({'message':'OTP code has expired !','success':'true'}, status=status.HTTP_200_OK)
