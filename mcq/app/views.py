        ...
        if 'id' in kwargs:
            ...
            if user.is_authenticated:
                ...
                if has_purchases:
                    ...
                    question_sets_queryset = QuestionSets.objects.filter(categorie=category, is_free=True)
                    question_sets_serializer = QuestionSetSerializers(question_sets_queryset, many=True)

                    response_data = {
                        "category": CategoriesSerailizers(category).data,
                        "question_sets": question_sets_serializer.data,
                        "success": True
                    }
                    return Response({'data': response_data}, status=status.HTTP_200_OK)
...
#question set related question and answer api
    ...
    def list(self, request, *args, **kwargs):
        ...
        if user.is_authenticated:
            ...
            if has_purchases:
                ...
                question_sets_instances = QuestionSets.objects.filter(id__in=purchased_question_sets)
...
class PaymentPatnerList(generics.ListAPIView):
    ...
    queryset= PaymentPatner.objects.all()
    # permission_classes = [IsAuthenticated]
    def list(self, request, *args, **kwargs):
